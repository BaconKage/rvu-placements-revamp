...
export default function Words({ text, as: Tag = "span", className = "", delay = 0, hi, glow, mark }) {
    ...
    <Tag ref={ref} className={`words ${className}`} style={{ "--d": `${delay}ms` }}>
      {parts.map((w, i) => (
        <span className="w" key={i} style={{ "--i": i }}>
          <span className={`${hi?.has(i) ? "w-hi" : ""} ${glow?.has(i) ? "w-glow" : ""} ${mark ? "w-mark" : ""}`.trim()}>{w}{i < parts.length - 1 ? " " : ""}</span>
        </span>
      ))}
    </Tag>
  );
